...
def trailer_tmdb(movie_id):
    """Busca un trailer de YouTube para la película. Prueba primero en español, después en el idioma original."""
    for params in ({"language": "es-419"}, {}):
        data = tmdb_get(f"movie/{movie_id}/videos", params) or {}
        ...
        elegido = oficiales[0] if oficiales else trailers[0]
        if elegido.get("key"):
            return elegido["key"]
    return None
...
@st.cache_data(ttl=86400, show_spinner=False)
def trailer_tmdb(movie_id):
    """Devuelve el key de YouTube del tráiler, o None si no encuentra ninguno."""
    if not movie_id:
        return None
    ...
    videos = _buscar_en("es-419") or _buscar_en("en-US")
...
# ============================================================
    ...
    if not deck:
        ...
        if actual.get("overview"):
            with st.expander("📖 Sinopsis", expanded=True):
                st.write(actual["overview"])

        with st.expander("🎬 Ver trailer"):
