...
def supabase_signup(email, password, nombre):
    try:
        r = requests.post(
    ...
    with tab_registro:
        ...
        if crear:
            if not nombre.strip() or not email_r.strip() or len(password_r) < 6:
                ...
                usuario, error = supabase_signup(email_r.strip(), password_r, nombre.strip())
                if usuario:
                    st.session_state.usuario = usuario
                    st.rerun()
                else:
...
st.sidebar.write("")
st.sidebar.caption(f"Conectado como **{st.session_state.usuario['nombre']}**")
