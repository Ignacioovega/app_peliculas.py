...
# ----------------------------------------
# Ficha de película con reseña editable
# ----------------------------------------
def render_ficha(p):
    if st.button("← Volver"):
        st.session_state.pelicula_activa = None
    ...
    with col_info:
        ...
        <span class="stat-chip"><span class="stat-label">Tu puntaje</span>
            <span class="stat-value">⭐ {p['puntaje']}/10</span></span>
        """
...
# ----------------------------------------
    ...
    st.subheader("Buscar en tu colección")

    texto = st.text_input("Buscá por título o género")

    if texto:
        resultado = [
            p for p in st.session_state.peliculas
            if texto.lower() in p["titulo"].lower() or texto.lower() in p["genero"].lower()
        ]
        if resultado:
            render_fila(f'Resultados para "{texto}"', resultado, key_prefix="buscar")
        else:
