...
def render_fila(titulo_fila, lista, key_prefix):
    ...
    key = f"select_{key_prefix}_{titulo_fila}"

    def _abrir_ficha_seleccionada(key=key):
        seleccion = st.session_state[key]
        if seleccion != "—":
            st.session_state.pelicula_activa = seleccion

    st.selectbox(
        "Ver ficha de:", ["—"] + titulos, key=key, on_change=_abrir_ficha_seleccionada
    )
...
# ----------------------------------------
    ...
    if texto.strip():
        ...
        if not resultados:
            ...
            def _abrir_desde_busqueda(opciones=opciones):
                seleccion = st.session_state["select_buscar_resultado"]
                if seleccion != "—":
            ...
            st.selectbox(
                "Ver ficha de:",
                ["—"] + list(opciones.keys()),
                key="select_buscar_resultado",
                on_change=_abrir_desde_busqueda,
            )
