...
RUTA_DATOS = "datos_cinevega.json"


def cargar_datos_guardados():
    """Lee la colección guardada en disco, si existe. Es compartida: la ve cualquiera que entre a la app."""
...
def guardar_datos():
    """Guarda la colección actual en disco para que sobreviva a un refresh."""
    try:
        with open(RUTA_DATOS, "w", encoding="utf-8") as f:
            json.dump(st.session_state.peliculas, f, ensure_ascii=False)
    except Exception:
        pass
...
st.markdown(
    ...
    .genero-badge {
        display: inline-block; background-color: rgba(60, 140, 255, 0.18); color: #9ec5ff;
        border: 1px solid rgba(60, 140, 255, 0.35); border-radius: 20px; padding: 2px 9px;
        font-size: 0.68rem; margin-right: 4px; margin-bottom: 4px; white-space: nowrap;
    }
    .puntaje-badge {
        display: inline-block; background: linear-gradient(120deg, #e50914, #ff4d67); color: white;
        border-radius: 20px; padding: 2px 9px; font-weight: 700; font-size: 0.68rem;
    ...
    /* ===== Vuelta de rosca visual ===== */

    /* Header más impactante, con textura sutil */
    .hero-header {
        position: relative;
        overflow: hidden;
    }
    ...
    .hero-header p { position: relative; z-index: 1; }

    /* Tarjetas de la grilla: cualquier columna que tenga un botón adentro
       se convierte en una tarjeta tipo "vidrio" con elevación al pasar el mouse */
    div[data-testid="column"]:has(button) {
        background: linear-gradient(160deg, #1c1f2e 0%, #14151f 100%);
    ...
    div[data-testid="column"]:has(button):hover {
        ...
        box-shadow: 0 16px 32px rgba(123, 47, 247, 0.28);
    ...
    div[data-testid="column"]:has(button):hover [data-testid="stImage"] img {
        transform: scale(1.03);
    }

    /* Jerarquía de botones: los principales (agregar, guardar) se destacan;
       el resto (ver ficha, volver, cargar más) quedan sutiles tipo "ghost" */
    ...
    button[kind="primary"] {
        background: linear-gradient(120deg, #e50914, #7b2ff7) !important;
        border: none !important;
    }

    /* Resalte al pasar el mouse por las opciones del menú lateral */
    section[data-testid="stSidebar"] label:hover {
        background: rgba(123, 47, 247, 0.12);
        border-radius: 8px;
    }

    /* Sinopsis con look de tarjeta */
...
def pelicula_desde_resultado_tmdb(resultado, detalle=None):
    ...
    descripcion = (detalle.get("overview") if detalle else None) or resultado.get("overview") or ""
...
if "peliculas" not in st.session_state:
    ...
    if datos_guardados:
        st.session_state.peliculas = datos_guardados
    else:
...
def render_ficha(p):
    ...
    with col_info:
        ...
        with st.form(f"form_resena_{p['titulo']}"):
            ...
            guardar = st.form_submit_button("Guardar reseña", type="primary")
...
def render_grid(lista, key_prefix, columnas=6):
    """Grilla de posters con un botón real debajo de cada uno para abrir la ficha."""
    for i in range(0, len(lista), columnas):
        ...
        for col, p in zip(cols, fila):
            with col:
                st.image(p.get("poster") or PLACEHOLDER, use_container_width=True)
                titulo_corto = p["titulo"] if len(p["titulo"]) <= 22 else p["titulo"][:20] + "…"
                st.markdown(f"**{titulo_corto}**")
                rating = p["tmdb_rating"] if p.get("tmdb_rating") else "—"
                badges = f'<span class="genero-badge">🌐 {rating}/10</span>'
                if p.get("calificada"):
                    badges += f' <span class="puntaje-badge">⭐ {p["puntaje"]}/10</span>'
                st.markdown(compactar_html(badges), unsafe_allow_html=True)
                if st.button("Ver ficha", key=f"ver_{key_prefix}_{p['titulo']}", use_container_width=True):
...
opcion_menu = st.sidebar.radio(
    "Menú",
    ["🏠 Inicio", "➕ Agregar película", "🎬 Mi colección", "🔍 Buscar", "📊 Estadísticas"],
)
...
# ----------------------------------------
    ...
    if seleccionada:
        ...
        with col_info:
            ...
            descripcion = (detalle.get("overview") if detalle else "") or ""
...
# ----------------------------------------
    ...
    if not peliculas:
        st.info("Todavía no hay películas cargadas")
    else:
        ...
        # --- Filtro por género + orden (estilo JustWatch/Letterboxd) ---
        generos_disponibles = sorted({p["genero"].split(",")[0].strip() for p in peliculas})
        col_filtro, col_orden = st.columns(2)
        with col_filtro:
            genero_elegido = st.selectbox("Filtrar por género", ["Todos"] + generos_disponibles)
        with col_orden:
            orden_elegido = st.selectbox(
                "Ordenar por",
                ["Más popular (TMDb)", "Tu puntaje", "Alfabético", "Recién agregada"],
            )

        peliculas_filtradas = (
            peliculas
            if genero_elegido == "Todos"
            else [p for p in peliculas if p["genero"].split(",")[0].strip() == genero_elegido]
        )

        if orden_elegido == "Más popular (TMDb)":
            peliculas_filtradas = sorted(peliculas_filtradas, key=lambda p: p["tmdb_rating"], reverse=True)
        elif orden_elegido == "Tu puntaje":
            peliculas_filtradas = sorted(peliculas_filtradas, key=lambda p: p["puntaje"], reverse=True)
        elif orden_elegido == "Alfabético":
            peliculas_filtradas = sorted(peliculas_filtradas, key=lambda p: p["titulo"].lower())
        elif orden_elegido == "Recién agregada":
            peliculas_filtradas = list(reversed(peliculas_filtradas))

        st.caption(f"{len(peliculas_filtradas)} película(s) en el catálogo")
        render_grid(peliculas_filtradas, key_prefix="inicio")
...
# ----------------------------------------
# Mi colección (solo lo que vos calificaste y comentaste)
# ----------------------------------------
elif menu == "Mi colección":
    calificadas = [p for p in st.session_state.peliculas if p.get("calificada")]

    if not calificadas:
        st.info("Todavía no calificaste ninguna película. Andá a '🏠 Inicio', elegí una y ponele tu puntaje/comentario en su ficha.")
    else:
        destacada = max(calificadas, key=lambda p: p["puntaje"])
        imagen_banner = destacada.get("backdrop") or destacada.get("poster")
        if imagen_banner:
            st.markdown(
                ...
                <div class="featured-banner" style="background-image: url('{imagen_banner}');">
                    <div class="featured-content">
                        ...
                        <h2>{destacada['titulo']}</h2>
                        <p>{destacada['genero']} · {destacada['duracion']} min · Tu puntaje {destacada['puntaje']}/10</p>
                    </div>
                </div>
                """),
                unsafe_allow_html=True,
        ...
        generos_disponibles = sorted({p["genero"].split(",")[0].strip() for p in calificadas})
        col_filtro, col_orden = st.columns(2)
        with col_filtro:
            genero_elegido = st.selectbox("Filtrar por género", ["Todos"] + generos_disponibles)
        with col_orden:
            orden_elegido = st.selectbox(
                "Ordenar por", ["Tu puntaje", "Alfabético", "Recién calificada"]
            )

        vistas = (
            calificadas
            if genero_elegido == "Todos"
            else [p for p in calificadas if p["genero"].split(",")[0].strip() == genero_elegido]
        )

        if orden_elegido == "Tu puntaje":
            vistas = sorted(vistas, key=lambda p: p["puntaje"], reverse=True)
        elif orden_elegido == "Alfabético":
            vistas = sorted(vistas, key=lambda p: p["titulo"].lower())
        elif orden_elegido == "Recién calificada":
            vistas = list(reversed(vistas))

        st.caption(f"{len(vistas)} película(s) que calificaste")
        render_grid(vistas, key_prefix="micoleccion")
    ...
    if texto.strip():
        ...
        if not resultados:
            ...
            for i in range(0, len(resultados), columnas):
                fila = resultados[i : i + columnas]
                cols = st.columns(columnas)
                for col, r in zip(cols, fila):
                    with col:
                        ...
                        st.caption(f"📅 {anio}")
                        if st.button("Ver ficha", key=f"ver_buscar_{r['id']}", use_container_width=True):
                            if not existe_titulo(r["title"]):
                                with st.spinner("Cargando datos..."):
                                    nueva = pelicula_desde_resultado_tmdb(r)
                                    st.session_state.peliculas.append(nueva)
                            st.session_state.pelicula_activa = r["title"]
                            st.rerun()
